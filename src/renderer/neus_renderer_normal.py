...
class NeuSRenderer:
    ...
    def up_sample(self, rays_o, rays_d, z_vals, sdf, n_importance, inv_s):
        ...
        with torch.cuda.device(0):
            ...
            # |
            # ----------------------------------------------------------------------------------------------------------
            #print(rays_o.device,sdf.device)
            prev_cos_val = torch.cat([torch.zeros([batch_size, 1],device=rays_o.device), cos_val[:, :-1]], dim=-1).to(rays_o.device)
    ...
    def render_core(self,
        ...
        with torch.cuda.device(0):
            ...
            sampled_color = color_network(pts, gradients, dirs, feature_vector,ratio).reshape(batch_size, n_samples, 3)
    ...
    def render(self, rays_o, rays_d, near, far, perturb_overwrite=-1, background_rgb=None, cos_anneal_ratio=0.0,ratio=0.):
        with torch.cuda.device(0):
    ...
    def forward(self,K,Rt,iters=0,resolution=(64,64)):
        device=K.device
        h,w=resolution
        ...
        grad_err=result["gradient_error"]
        sdf=result["sdf"]
        pts=result["pts"]
        return color.reshape(h,w,3),grad_err,sdf,pts
