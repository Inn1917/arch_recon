...
from src.dataset.rand_cam import CamDataset,CamDataset_R
...
if __name__=="__main__":
    ...
    color_network=RenderingNetwork_NF(**conf['model.rendering_network']).to(device)
    ...
    #sdf_network.load_state_dict(torch.load("./saved_model/15000_sdf.pth"))
    #color_network.load_state_dict(torch.load("./saved_model/15000_color.pth"))
    ...
    dataset=CamDataset_R(cam_size=100)
    L=len(dataset)
    for i in range(opt.max_iter):
        K,Rt=dataset[i%L]
        K=K.to(device)
        Rt=Rt.to(device)
        color,loss_grad,sdf,pts=renderer.forward(K,Rt,iters=i)
        ...
        if i%500==0:
            ...
            cv2.imwrite(f"./result/img_nf/{i:03d}.png",img)
            ...
            mesh.export(os.path.join("./result/mesh_nf", f'{i:03d}.ply'))
        if i%3000==0:
            torch.save(sdf_network.state_dict(),f"./saved_model/{i:05d}_sdf_nf.pth")
            torch.save(color_network.state_dict(),f"./saved_model/{i:05d}_color_nf.pth")
