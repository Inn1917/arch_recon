...
from src.models.neus_sdf import SDFNetwork,RenderingNetwork,NeRF,SingleVarianceNetwork
from src.renderer.neus_renderer import NeuSRenderer

from src.dataset.rand_cam import CamDataset,CamDataset_FR
...
if __name__=="__main__":
    ...
    #conf["model.rendering_network.d_in"]=9
    #print(conf["model.rendering_network.d_in"])
    ...
    nerf_outside = None#NeRF(**conf['model.nerf']).to(device)
    #sdf_network.load_state_dict(torch.load("./saved_model_ord/27000_sdf.pth"))
    #color_network.load_state_dict(torch.load("./saved_model_ord/27000_color.pth"))
    ...
    params_to_train += list(variance_network.parameters())
    ...
    dataset=CamDataset(cam_size=40)
    L=len(dataset)
    for i in range(opt.max_iter):
        K,Rt,theta,phi=dataset[i%L]
        K=K.to(device)
        Rt=Rt.to(device)
        #color,loss_grad,_,_=renderer.forward(K,Rt,iters=i)
        ...
        color,loss_grad=renderer.forward(K,Rt)
        ...
        loss_all=loss_sds+5e-4*loss_grad#+5e-2*loss_bnd
        loss_all.backward()
        ...
        if i%500==0 or i==200:
            ...
            cv2.imwrite(f"./result/img_ord/{i:03d}.png",img)
            min_=torch.from_numpy(np.array([-2]*3))
            max_=torch.from_numpy(np.array([2]*3))
            v,t=renderer.extract_geometry(min_,max_,128)
            mesh = trimesh.Trimesh(v, t)
            mesh.export(os.path.join("./result/mesh_ord", f'{i:03d}.ply'))
        if i%3000==0:
            torch.save(sdf_network.state_dict(),f"./saved_model_ord/{i:05d}_sdf.pth")
            torch.save(color_network.state_dict(),f"./saved_model_ord/{i:05d}_color.pth")
